...
resource "null_resource" "deploy_stack" {
  ...
  provisioner "local-exec" {
    command = "sed -i '' -e 's#APIBase:.*#APIBase: \"//${var.env_ip}:8080\"#g' ../client/src/environments/environment.prod.ts"
  }
  provisioner "local-exec" {
    command = "ansible-playbook -e public_ip='${var.env_ip}' deploy.yaml -vvv"
  }
}
